...
for q = 1 : 20
    while counter < run
                            ...
                            if CW(src) == 0
                                ...
                                if length(b) == 1
                                    if ~ismember(idx,successfulRelays) && relayState(idx)== 0
                                        ...
                                        relayState(idx) = 0;
                                        E1(idx)=E1(idx)+ P*TB*Ir*V; % energy for receiving m pkts
                    ...
                    while NB(src) <= macMaxCSMABackoffs
                        ...
                        if (w > (q / 100))
                            ...
                            if CW(src) == 0
                                ...
                                pktsTransmitted(src) = pktsTransmitted(src) + 1;
                                srcPktsTXed(src) = srcPktsTXed(src) + 1;
                                E1(src)=E1(src)+ (P*TB*It*V); %energy cost for transmiting m packets
                                            ...
                                            if ~ismember(idx,reservedCaseRelays) && relayState(idx) == 0
                                                ...
                                                if ~ismember(idx,reservedCaseRelays) && relayState(idx) == 0
                                                    ...
                                                    if ~ismember(idx,reservedCaseRelays) && relayState(idx) == 0
                                                        ...
                                                        fprintf('\nReserved Channel Case: Relay %d of Source %d replies with an Ack',idx,src);
                                                        reservedCaseRelays(idx) = idx;
                                                        E1(idx)=E1(idx)+ mP*TB*Ir*V; % energy for receiving m pkts
                                                        %E1(idx)=E1(idx)+ Ack*TB*It*V; % energy cost for transmitting an ack to source
                                                        relayEnergy(idx) = E1(idx);
                                                        %pktsTransmitted(idx) = pktsTransmitted(idx) + 1;
                                                        RpktsTransmitted(idx) = RpktsTransmitted(idx) + 1;
                ...
                while NB(src) <= macMaxCSMABackoffs
                    ...
                    if (w > (q / 100))
                        ...
                        if CW(src) == 0
                            ...
                            E1(src)=E1(src)+ (m*P*TB*It*V); %energy cost for transmiting m packet
                            pktsTransmitted(src) = pktsTransmitted(src) + 1;
                            srcPktsTXed(src) = srcPktsTXed(src) + 1;
                            for l = 1:sizeT
                                for k=1:sizeR% size of all relays
        ...
        for k = 1 : sizeRelays
            ...
            E1(indx)=E1(indx)+ relaySlotAssigned(k) * (P*TB*It*V); % energy for transmitting m pkts
...
%ZIGBEE MAC with Opportunistic relaying
...
for q = 1:20
    while counter < run
        ...
        trans = sort(ia);
        sortedTransmitters = sources(trans);
        %fprintf('Opportunistic Routing of ZIGBEE Case: Sorted backoff: ');
        sortedBackoff = backoff(trans);
        %disp(sortedBackoff);
        D = D * 100;
        sizeSB = length(sortedBackoff);
        ...
        %fprintf('\nOpportunistic Routing of ZIGBEE Case; ');
        %disp(baseChannel);
        %initialize the sources backoff array
        for g =1:S
            csmaTime(g) = backoff(g);
        ...
        for i = 1:S % for all sources
            src = sources(i);
            if ismember(src,transmitters)
                NB(src) = 0;
                CW(src) = 2;
                csmaTime(src) = backoff(i)*BP + TCCA;
                E2(src)=E2(src)+ TCCA*Il*V; % stays listening for TCCA time
                fprintf('\nOpportunistic Routing of ZIGBEE Case; Node %d performs CCA', src);
                while NB(src) <= macMaxCSMABackoffs
                    %  if baseChannel(src) == 1
                    w = rand();
                    if (w > (q/100))
                        ...
                        if CW(src) == 0
                            ...
                            pktStartTXTime(src) = csmaTime(src) + ST + SIFS; % pkts start transmission time
                            pktEndTXTime(src) = pktStartTXTime(src) + m*P*TB; % pkts end transmission time
                            fprintf('\nOpportunistic Routing of ZIGBEE Case; Node %d transmits data packet', src);
                            E2(src)=E2(src)+ m*P*TB*It*V ;
                            ...
                            if length(b) == 1
                                if ~ismember(idx,successfulRelays) && relayState(idx)== 0
                                    pktStartTXTime(idx) = pktEndTXTime(src) + ST + aTurnaroudTime + SIFS ; % pkts start transmission time
                                    pktEndTXTime(idx)= pktStartTXTime(idx) + Ack*TB + SIFS; % pkts end transmission time
                                    fprintf('\nOpportunistic Routing of ZIGBEE Case;Relay %d of Source %d replies with an Ack',idx,src);
                                    baseChannel(src) = 0;
                                    CW(src) = 2;
                                    NB(src) = 0;
                                    successfulRelays(idx) = idx;
                                    relayState(idx) = 0;
                                    E2(idx)=E2(idx)+ SIFS*Il*V;
                                    E2(idx)=E2(idx)+ 2*(m+1)*SE + m*(macACKWaitDuration*Il*V);
                                    E2(idx)=E2(idx)+ m*P*TB*Ir*V; % energy for receiving m pkts
                                    E2(idx)=E2(idx)+ m*Ack*TB*It*V; % energy cost for transmitting an ack to source
                                    E2(idx)=E2(idx)+ m*P*TB*It*V;
                                    relayEnergy(idx) = E2(idx);
                                    pktsTransmitted(idx) = pktsTransmitted(idx) + 1;
                                    RpktsTransmitted(idx) = RpktsTransmitted(idx) + 1;
                                    break;
                                else
                                    fprintf('\nOpportunistic Routing of ZIGBEE Case;Source %d could not find any relays, "Packet lost"', src);
                                    baseChannel(src) = 0;
                                    CW(src) = 2;
                                    NB(src) = 0;
                                    pktsLost(src) = pktsLost(src) + 1;
                                ...
                                if length(b) == 2
                                    ...
                                    fprintf('\nOpportunistic Routing of ZIGBEE Case;Relay Number exceeds MaxNumberRelays = 5');
                                    pktsLost(src) = pktsLost(src) + 1;
                                end %end length(b) == 2
                            end %end length(b) == 1
                            break;
                            ...
                            csmaTime(src) = csmaTime(src) + TCCA;
                            fprintf('\nOpportunistic Routing of ZIGBEE Case;Node %d performs CCA,%d', src,CW(src));
                            E2(src)=E2(src)+ TCCA*Il*V; % stays listening for TCCA time
                        end
                    else
                        CW(src) = 2;
                        ...
                        if NB(src) <= macMaxCSMABackoffs
                            ...
                            csmaTime(src) = backoff(i)*BP + TCCA;
                            E2(src)=E2(src)+ TCCA*Il*V; % stays listening for TCCA time
                            fprintf('\nOpportunistic Routing of ZIGBEE Case; NB[%d] = %d', src,NB(src));
                        else
                            fprintf('\nOpportunistic Routing of ZIGBEE Case; Node %d terminates; NB[ %d ] exceeds macMaxCSMABackoffs = 5', src, src);
            ...
            while NB(src) <= macMaxCSMABackoffs
                ...
                if (w > (q / 100))
                    ...
                    if CW(src) == 0
                        ...
                        E3(src)= E3(src)+ m*P*TB*It*V+ m*Ack*TB*Ir*V + (m+1)*SE + macACKWaitDuration*Il*V;
                        pktsTr(src) = pktsTr(src) + 1;
    ...
    while counter < run
        for j = 1 : N
            if j == 1
                ...
                E4(j) = E4(j) + m*P*TB*It*V + m*Ack*TB*It*V + 2*m * SE + m * macACKWaitDuration * Il * V ;
            end
        end
        counter = counter + 1;
    end
...
hold on
grid on
legend('Energy Consumption "IAA Algo"','Energy Consumption "OR"','Energy Consumption "Traditional Zigbee"', 'TDMA Scheme')
xlabel('Node Index');
